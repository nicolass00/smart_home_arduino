...
void RefreshTemperature(){
    Temperatura1 = sensorTemp.getTempC(insideThermometer);
...
void OutControl()
{
  ...
  //Caleraccion Zona 1
  // Heating Zone 1
  if (ElectricalCircuitValue[17]==1){
    //Calefacción encendida comprobar histeresis
    //Heating on, test temperature
    float RangoTemp = ElectricalCircuitValue[20] + 0.7;
    if (Temperatura1   >= RangoTemp )
    {
      //apagamos calefaccion
      //turn off heating
      digitalWrite(PinOutput[10], LOW);     
    }
    else  //encendemos calefaccion  turn on heating
    {  
      RangoTemp = RangoTemp - 1.4;
      if (Temperatura1 <= RangoTemp){digitalWrite(PinOutput[10], HIGH); }
    }
  }
  else{digitalWrite(PinOutput[10], LOW);}
...
char* ReadSensor(byte NumeroSensor)
{
  ...
  //
  if (NumeroSensor==1){return "22,5º";}
...
void loop(){
   ...
   if((millis() - LastMsg) >= 30000) {CargaHora();RefreshTemperature();if (EspRfrIp<1){connectAndRfr();EspRfrIp=120;}else{EspRfrIp--;}}
